...
// 将 4 个口令的填充块转置为16个NEON向量
// blocks:长度为4的指针数组，每个指向 64 字节填充块
// x_neon:输出16个向量，每个向量包含4个口令的第i个word
static void TransposeTo4(const Byte* blocks[4], uint32x4_t x_neon[16]) {
	for (int i = 0; i < 16; i++) {
		// 从 4 个块中提取32位字，组成一个NEON向量
		uint32_t words[4];
		for (int j = 0; j < 4; j++) {
			memcpy(&words[j], blocks[j] + i * 4, 4);
		}
		x_neon[i] = vld1q_u32(words);
	}
}

/**
 * 4 路并行NEON MD5哈希函数
 * @param batch 包含4个字符串的向量
 * @param results 长度为16的数组，存储4个结果的 A,B,C,D
 */
void MD5Hash_NEON_4(const std::vector<string>& batch, uint32_t* results) {
	// // 对 4 个口令分别进行填充
	// alignas(16) Byte padded_blocks[4][64];
	// const Byte* block_ptrs[4];

	// for (int i = 0; i < 4; i++) {
	//     memset(padded_blocks[i], 0, 64);
	//     int len = batch[i].length();
	//     memcpy(padded_blocks[i], batch[i].c_str(), len);
	//     padded_blocks[i][len] = 0x80;
	//     uint64_t bitLength = (uint64_t)len * 8;
	//     memcpy(padded_blocks[i] + 56, &bitLength, 8);  // 注意：MD5 长度存储在小端序的 56~63 字节
	//     block_ptrs[i] = padded_blocks[i];
	// }

	// //数据转置
	// uint32x4_t x[16];
	// TransposeTo4(block_ptrs, x);


	   // SoA 缓冲区：16 个向量，每个向量4个数字
	// x[]4个连续的32位字分别来自4个口令的同一位置，方便NEON并行处理
	alignas(16) uint32_t soa_buffer[16 * 4];

	// 对 4 个口令分别填充，并直接写入 SoA 缓冲区
	for (int i = 0; i < 4; i++) {
		alignas(16) Byte padded[64] = { 0 };
		size_t len = batch[i].length();
		memcpy(padded, batch[i].c_str(), len);
		padded[len] = 0x80;
		uint64_t bitLen = len * 8;
		memcpy(padded + 56, &bitLen, 8);

		// 将填充后的 16 个 32 位字写入 SoA 缓冲区
		const uint32_t* words = reinterpret_cast<const uint32_t*>(padded);
		for (int j = 0; j < 16; j++) {
			soa_buffer[j * 4 + i] = words[j];
		}
	}
	...
	vst1q_u32(results, a);
	vst1q_u32(results + 4, b);
	vst1q_u32(results + 8, c);
	vst1q_u32(results + 12, d);
}
