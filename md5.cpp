...
static void TransposeTo4(const Byte* blocks[4], __m128i x_sse[16]) {
    for (int i = 0; i < 16; i++) {
        alignas(16) uint32_t words[4];
        for (int j = 0; j < 4; j++) {
            memcpy(&words[j], blocks[j] + i * 4, 4);
        }
        x_sse[i] = _mm_load_si128((__m128i*)words);
    }
}

/**
 * 4 路并行 SSE MD5 哈希核心函数
 * @param batch 包含 4 个字符串的向量
 * @param results 长度为 16 的 uint32_t 数组，存储 4 个结果的 A,B,C,D 依次排列
 */
void MD5Hash_SSE_4(const std::vector<string>& batch, uint32_t* results) {
    ...
    for (int i = 0; i < 4; i++) {
        ...
        memcpy(padded_blocks[i] + 56, &bitLength, 8);
    ...
    _mm_storeu_si128((__m128i*)(results), a);
    _mm_storeu_si128((__m128i*)(results + 4), b);
    _mm_storeu_si128((__m128i*)(results + 8), c);
    _mm_storeu_si128((__m128i*)(results + 12), d);
}
