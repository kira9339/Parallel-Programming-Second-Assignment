...
// 编译指令 (x86 平台)
// g++ -O2 -std=c++11 -mavx2 main.cpp train.cpp guessing.cpp md5.cpp -o main_avx2
...
int main()
{
    ...
    q.m.train("/mnt/d/copy_guess/Rockyou-singleLined-full.txt");
    q.m.order();
    ...
    // 存储哈希结果
    static uint32_t hash_dummy_buffer[32]; // 只需足够存放一次 8 路结果
    ...
    while (!q.priority.empty())
    {
        ...
        if (q.total_guesses - curr_num >= 100000)
        {
            ...
            // 在此处更改实验生成的猜测上限
            int generate_n = 20000000;
            if (history + q.total_guesses > 20000000)
            {
        ...
        {
            vector<string> batch;
            alignas(32) uint32_t results[32]; 
            auto start_hash = system_clock::now();

            for (const string& pw : q.guesses)
            {
                // 长度 <= 55 字节走 AVX2 加速（单块处理）
                if (pw.length() <= 55) {
                    batch.push_back(pw);
                    if (batch.size() == 8) {
                        MD5Hash_AVX2_8(batch, results);
                        memcpy(hash_dummy_buffer, results, 32 * sizeof(uint32_t));
                        batch.clear();
                    }
                }
                else {
                    ...
                    memcpy(hash_dummy_buffer, single_state, 4 * sizeof(bit32));
                }
            }
            // 处理不足 8 个的剩余口令
            if (!batch.empty()) {
