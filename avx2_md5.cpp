...
// 将8个口令的填充块转置为 16 个 AVX2 向量
static void TransposeTo8(const Byte* blocks[8], __m256i x_avx2[16]) {
    for (int i = 0; i < 16; i++) {
        alignas(32) uint32_t words[8];
        for (int j = 0; j < 8; j++) {
            memcpy(&words[j], blocks[j] + i * 4, 4);
        }
        x_avx2[i] = _mm256_load_si256((__m256i*)words);
    }
}
 ...
 * 8路并行AVX2 MD5哈希核心函数
 */
void MD5Hash_AVX2_8(const std::vector<string>& batch, uint32_t* results) {
    alignas(32) Byte padded_blocks[8][64];
    const Byte* block_ptrs[8];
    ...
    _mm256_storeu_si256((__m256i*)(results), a);
    _mm256_storeu_si256((__m256i*)(results + 8), b);
    _mm256_storeu_si256((__m256i*)(results + 16), c);
    _mm256_storeu_si256((__m256i*)(results + 24), d);
}
